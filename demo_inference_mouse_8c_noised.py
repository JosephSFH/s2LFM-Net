...
def parse_args():
    ...
    parser.add_argument("--angres", type=int, default=8, help="input angle number")

    parser.add_argument("--out_channels", type=int, default=8, help="output spectrum channels")
    parser.add_argument('--testset_dir', type=str, default='dataset/testset/mouse_spleen/8c_noised/')

    parser.add_argument("--patchsize", type=int, default=136, help="inference patches size")
    parser.add_argument("--stride", type=int, default=136, help="stride size two inference patches")
    parser.add_argument("--netChannel", type=int, default=128, help="the feature layers of network")

    parser.add_argument('--model_path', type=str, default='pretrain_model_demo/mouse_spleen/8c/Noised_Finetuned/epoch_150.pth.tar')
    parser.add_argument('--save_path', type=str, default='output/mouse_spleen/8c_noised/')
...
def test(cfg, test_Names, test_loaders):
    ...
    net = s2lfm_Net(cfg.angres, cfg.out_channels, cfg.netChannel)
    net.to(cfg.device)
