...
class s2lfm_Net(nn.Module):
    ...
    def __init__(self, angular_in, out_channels=4):
        ...
        self.Final = SAInteraction(channel, angular_in)
    ...
    def forward(self, x):
        ...
        Target = self.Final(buffer_mv)
        out = self.Out(Target.contiguous().view(b*self.angRes*self.angRes, -1, h, w))
        # Combine network output with shortcut connection
        out = LFtiling(out.contiguous().view(b,-1, self.out_channels, h, w)) + LFtiling(Conv_cat)

        return out

class SAInteraction(nn.Module):
    """
    Spatial-angular interaction module.
    """
    def __init__(self, channel, angular_in):
        super(SAInteraction, self).__init__()
        self.an = angular_in
